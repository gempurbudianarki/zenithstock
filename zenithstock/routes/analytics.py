...
@login_required
def index():
    """Halaman utama analitik inventaris"""
    ...
    movements_30d   = (StockMovement.query
    ...
    for i in range(13, -1, -1):
        ...
        masuk  = sum(m.jumlah for m in movements_30d
                     if m.created_at.date() == day.date() and m.tipe == 'MASUK')
        keluar = sum(abs(m.jumlah) for m in movements_30d
    ...
    # ── Critical Stock List ───────────────────────────────
    critical_products = [p for p in all_products if p.stok < p.min_stok]
...
@login_required
def chart_data():
    """API endpoint untuk refresh chart data secara AJAX"""
    all_products  = Product.query.all()
