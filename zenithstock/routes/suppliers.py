...
from zenithstock.models import Supplier
...
def index():
    ...
    # Count products linked to suppliers
    from zenithstock.models import Product
    products_with_supplier = Product.query.filter(Product.supplier_id != None).count()
...
def delete(supplier_id):
    # Only Admin is authorized to delete suppliers
    ...
    try:
        ...
        db.session.rollback()
        flash(f'Gagal menghapus supplier "{nama}". Kemungkinan supplier ini sedang digunakan oleh data barang.', 'danger')
